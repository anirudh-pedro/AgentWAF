...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="h-screen w-screen bg-slate-100 text-slate-900 font-sans flex flex-row overflow-hidden">
      {/* Dark Vertical Left Sidebar containing Agent WAF System Health Component */}
      <Sidebar health={health} onOpenUserQuery={() => setIsQueryModalOpen(true)} />

      {/* Main Content Area — Single Outer Page Scrollbar */}
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        {/* Top Header */}
        <header className="bg-white border-b border-slate-200 px-4 md:px-5 py-3 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-2xs shrink-0">
          <div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded text-xs font-semibold flex items-center gap-1 transition border border-slate-300"
            >
              <RefreshCw className="w-3.5 h-3.5" /> Refresh
            </button>
            <span className="text-[11px] font-mono font-bold text-slate-700 border border-slate-300 bg-slate-50 px-2.5 py-1 rounded">
        ...
        <main className="p-3 md:p-4 space-y-4 max-w-[1400px] mx-auto w-full">
          ...
          <div className="bg-[#1E293B] border border-slate-700 rounded-lg p-3 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-2xs text-white">
            <div className="flex items-center space-x-2.5">
              <div className="w-7 h-7 rounded bg-blue-600 text-white flex items-center justify-center shrink-0 shadow-2xs">
            ...
            <button
              ...
              className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded text-xs font-bold flex items-center gap-1.5 transition shadow-2xs shrink-0"
            >
              <MessageSquare className="w-3.5 h-3.5" /> Open User Query
            </button>
          </div>

          {/* API Failure Banner */}
          {isGlobalError && (
            <div className="bg-rose-50 border border-rose-200 rounded-lg p-3 flex items-center justify-between text-rose-800 shadow-2xs text-xs">
              <div className="flex items-center space-x-2">
          ...
          {/* Bottom Section: Audit Timeline (8 cols) + Collectors Bar Chart (4 cols) */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 md:gap-4 pb-6 items-start">
            ...
            {/* Right Column (4 Columns): Collectors & Tool Calls Bar Chart */}
